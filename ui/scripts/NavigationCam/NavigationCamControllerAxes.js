var navigationCamControllerAxes = (function() {
    	
	//TODO Statt alles auf eigene View und Cam Matrix zu setzen, sollte alles über den Viewpoint laufen
	//		-> Es müssten animateTo und auch Tick nicht übersteuert werden
	//		-> Es würden mehrere Viewpoints möglich sein und die Ermittlung der Cam ist genauso simpel	
	// 	Dann aber auch alles ohne viewarea, im grunde eigentlich eine eigene viewarea bauen und ersetzen



	// var actionEventMapping = {
	// 	action: mouseMoveLeft,
	// 	event:	rotateCamY,
	// 	constraints: [
	// 		mouesKeyPressed[1],
	// 		keyBoardKeyPressed[62],
	// 		mouseKeyNotPressed[2],
	// 	]
	// }
		...
		if(!isMouseDown){
			...
			if(controllerConfig.setCenterOfRotation && !controllerConfig.setCenterOfRotationFocus){
				if(eventObject.entity){
					...
					// canvasManipulator.setCenterOfRotation(eventObject.entity, false);
				}
			}
		...
		if(controllerConfig.setCenterOfRotation && controllerConfig.setCenterOfRotationFocus && !mouseMoved){
			...
			if(eventObject.entity){
				...
				if(mousePressedTime < 100){
					...
					// canvasManipulator.setCenterOfRotation(eventObject.entity, true);
				}
