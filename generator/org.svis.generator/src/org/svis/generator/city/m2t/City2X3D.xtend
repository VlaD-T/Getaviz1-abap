...
class City2X3D {
	...
	def toX3DBody(Resource resource) {
		...
		val City2X3D_abap c2x_abap = new City2X3D_abap()
		val entities = EcoreUtil2::getAllContentsOfType(resource.contents.head, Entity)
		
		if (config.parser == FamixParser::ABAP) {
			return c2x_abap.run(entities)
		} else {
			return run(entities)
		}
	}
	
	def private run(List<Entity> entities) {
	...
	def String toX3DModel(List<Entity> entities) '''
  		«FOR entity : entities»
			«IF entity.type == "FAMIX.Namespace"»
				«toDistrict(entity)»
			«ENDIF»
			«IF entity.type == "FAMIX.Class" || entity.type == "FAMIX.Method" »
